...
| Date | Provider | Model | Source Mode | Sample ID | Generation Mode | Schema Result | Quality Result | Product Flow Result | Issues | Next Action | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-05-04 | deepseek | deepseek-chat | idea | idea_safe_sample_001 | llm | pass | usable_with_minor_issues | pass | LLM-QA-001 | Prompt should preserve user-provided project_title when present. | Safe fictional sample; output summary only. |
| 2026-05-04 | deepseek | deepseek-chat | film_script | film_script_safe_sample_001 | llm | pass | usable_with_minor_issues | pass | LLM-QA-002 fixed | Prompt update and backend contract guardrail validated; target_episode_count=3 now returns 3 episodes. | Safe fictional sample; output summary only. Title prefix was still normalized from “测试短剧：旧影院来信” to “旧影院来信”; monitor project_title preservation with LLM-QA-001. |
| 2026-05-04 | deepseek | deepseek-chat | novel | novel_safe_sample_001 | llm | pass | usable_with_minor_issues | pass | LLM-QA-001 observation | Novel sample passed; monitor title preservation and hallucination scope in future prompt tuning. | Safe fictional sample; output summary only. target_episode_count=3 returned 3 episodes. |
...
| LLM-QA-001 | deepseek | idea | User-provided project title was rewritten from “测试短剧：旧楼灯火” to “旧楼遗稿”. | Prompt allows model to freely refine title. | low | Update script generation prompt to prefer preserving explicit project_title. | open |
| LLM-QA-002 | deepseek | film_script | target_episode_count not strictly followed; requested 3 episodes but model returned 1 episode. | Do not paste full output. | medium | Strengthened film_script_to_short_drama_v1 prompt and added backend target_episode_count contract validation. | fixed |
...
DeepSeek 三入口真实小样本已完成：idea / film_script / novel 均可返回合法 `ShortDramaScriptOutput`；film_script 的 `target_episode_count` 问题已通过 prompt update + backend contract guardrail 修复。
