...
- Internal Auth first-layer loop 已完成：schema → password policy → in-memory auth service → auth endpoints → frontend login API；
...
Auth 当前仍是安全虚构账号和开发期 in-memory service，不代表已完成持久化用户、`password_hash`、基于角色的授权或生产 token 策略。
...
- Auth persistence, Usage Ledger persistence, Deployment Runbook, environment variables, HTTPS/CORS, logging redaction, and permission boundary；
...
- 真实 Word `.docx` 导入预览回归；
- Auth 持久化、Usage Ledger 持久化、部署 Runbook、环境变量、HTTPS/CORS、日志脱敏和权限边界；
