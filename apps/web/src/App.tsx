...
const defaultScriptSegmentationForm: ExistingScriptInput = {
  project_title: "测试短剧：雨夜重逢",
  script_text:
    "第1集 第1场｜医院门口｜雨夜。暴雨中，林晚撑着黑伞站在医院门口，顾沉从车里下来，两人隔雨对视。顾沉：你终于肯回来了？林晚：我回来，不是为了见你。",
...
function App() {
  ...
  const [scriptSegmentationForm, setScriptSegmentationForm] =
    useState<ExistingScriptInput>(defaultScriptSegmentationForm);
  ...
  const handleScriptSegmentationSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      pushToast("success", "切分完成", "已有剧本已生成 mock 结构化片段。");
    } catch {
  ...
  const copyScriptSegmentationJson = async () => {
    ...
    try {
      ...
      setScriptSegmentationExportStatus("");
      setScriptSegmentationError("");
      pushToast("success", "复制成功", "已有剧本切分 JSON 已复制到剪贴板。");
    } catch {
      setScriptSegmentationError("复制切分 JSON 失败，请检查浏览器剪贴板权限。");
      pushToast("error", "复制失败", "复制已有剧本切分 JSON 失败，请检查浏览器剪贴板权限。");
    }
  };
  ...
  const exportScriptSegmentationJson = () => {
    ...
    setScriptSegmentationCopyStatus("");
    setScriptSegmentationError("");
    pushToast("success", "导出成功", "已有剧本切分 JSON 已导出。");
  };
