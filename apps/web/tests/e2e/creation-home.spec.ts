...
test.describe("Dramora creation home smoke", () => {
  ...
  test("submits context options for all script creation entries", async ({ page }) => {
    ...
    expect(capturedPayloads.get("film_script")?.context_options?.project_id).toBe(authProjectId);
    ...
    for (const payload of capturedPayloads.values()) {
      ...
      expect(payload.context_options?.project_id).toBe(authProjectId);
      expect(payload.context_options?.session_id).toBe(authSessionId);
